...
const mockFetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
  ...
  const chunks = [
    // First audio chunk
    JSON.stringify({ code: 0, data: Buffer.from('audio-data-1').toString('base64') }),
    // Sentence chunk for progress
    JSON.stringify({ code: 0, sentence: { text: '你好', words: [] } }),
    // Second audio chunk
    JSON.stringify({ code: 0, data: Buffer.from('audio-data-2').toString('base64') }),
    // End chunk
...
describe('Integration: download-mode', () => {
  ...
  const customOutputFile = join(testDir, 'custom.mp3')
  ...
  describe('complete download flow (mocked fetch)', () => {
    ...
    it('API error with non-zero code throws error containing message, no file written', async () => {
      const errorFetch = async (_input: RequestInfo | URL, _init?: RequestInit): Promise<Response> => {
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(new TextEncoder().encode(
              JSON.stringify({ code: 40001, message: 'Invalid credentials' }) + '\n'
            ))
            controller.close()
          },
        })
